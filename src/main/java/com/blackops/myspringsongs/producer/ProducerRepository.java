...
public interface ProducerRepository extends JpaRepository<Producer, Long> {
    ...
    @Transactional
    @Modifying
    @Query("UPDATE Producer p SET p.firstName = :#{#producer.firstName}, p.lastName = :#{#producer.lastName}, " +
            "p.address = :#{#producer.address}, p.city = :#{#producer.city}, p.state = :#{#producer.state} WHERE p.id = ?1")
    /**doc
